...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask, input_mask, compound):
        ...
        enc_outputs = enc_outputs*batch_mask
        ...
        enc_outputs = self.pos_ffn(enc_outputs,compound) # enc_outputs: [batch_size, d_model]
        return enc_outputs, pos_weights

#====================================================================================================#
class SQembSAtt_CPenc_Model(nn.Module):
    def __init__(self, d_model, d_k, n_heads, d_v, out_dim, cmpd_dim, d_ff):
        super(SQembSAtt_CPenc_Model, self).__init__()
        self.layers = EncoderLayer(d_model, d_k, n_heads, d_v, out_dim, cmpd_dim, d_ff)
...
class SQembWtLr_CPenc_Model(nn.Module):
    def __init__(self, d_model, cmpd_dim, d_ff, dropout):
        super().__init__()
    ...
    def forward(self, enc_inputs, input_mask, compound):
        ...
        input_mask: [batch_size, src_len]
        '''
        position_weight = nn.Softmax(dim=1)(self.weight(enc_inputs)*input_mask.masked_fill_(input_mask == 0, -np.inf).unsqueeze(2)).permute(0,2,1) #[batch_size, 1, src_len]
